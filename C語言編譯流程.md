...
ELF（Executable and Linkable Format）是一種用於儲存可執行檔案、目標檔案及共享庫的文件格式，廣泛應用於類 UNIX 系統（如 Linux）。

ELF 檔案包含以下主要區段：

**1. 代碼區段 (Text Segment)**：儲存程式的機器碼。

**2. 數據區段 (Data Segment)**：儲存靜態數據，如全域變數和靜態變數的初始值。

**3. 符號表 (Symbol Table)**：列出所有變數和函數的名稱及位址，用於連結和調試。

**4. 重定位表 (Relocation Table)**：記錄在連結階段需要調整的地址，用來正確載入程式碼和數據到內存中。
